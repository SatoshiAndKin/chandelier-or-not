...
contract StakedTokenFactory {
    event StakedTokenCreated(address indexed stakedToken, address indexed asset, address indexed vault, address indexed earnings);
    ...
    function createStakedToken(ERC4626 vault, address earnings) public returns (StakedToken stakedToken) {
        ...
        stakedToken = new StakedToken{salt: bytes32(0)}(asset, earnings, twabController, vault);

        emit StakedTokenCreated(address(stakedToken), address(asset), address(vault), earnings);
    }
}
